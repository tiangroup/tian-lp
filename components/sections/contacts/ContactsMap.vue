  ...
  <div class="contacts__map">
    <client-only>
      <yandex-map
        ...
        v-if="showMap"
        :show-all-markers="zoomCorrect"
      >
        <ymap-marker
          v-for="item in items.filter(i => i.coords)"
          :key="item.id"
          :coords="getCoords(item.coords)"
          :marker-id="item.id"
          :hint-content="item.title"
        >
          <div class="map__balloon" slot="balloon">
            <div class="map__balloon__title" v-if="item.title">
              {{ item.title }}
            </div>
            <div class="map__balloon__body">
              <div class="addresses connect__item" v-if="item.address">
              ...
              <div class="emails connect__item" v-if="item.email">
                <div class="connect__row">
                  <div class="connect__icon">
                    <svg
                      width="11"
                  ...
                  <div class="connect__instances">
                    <div class="connect__instance" v-html="item.email"></div>
                  </div>
                </div>
              </div>
              <div class="phones connect__item" v-if="item.phone">
                <div class="connect__row">
                  <div class="connect__icon">
                    <svg
                      width="8"
                  ...
                  <div class="connect__instances">
                    <div class="connect__instance" v-html="item.phone"></div>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    zoomCorrect() {
      if (this.items.filter(i => i.coords).length > 1) {
        return true;
      }
      return false;
    },
  ...
  methods: {
    processMap(map) {
      if (map) {
        ...
        let customColor = getComputedStyle(
